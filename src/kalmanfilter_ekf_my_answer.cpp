...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
        ...
        {           
            ...
            // Predicet Maesurement Vector (Measurement Model)
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        // ----------------------------------------------------------------------- //
        // ENTER YOUR CODE HERE
        VectorXd u = Vector4d::Zero();
        double V = state(3);
        double psi = state(2);
        
        u << V*cos(psi), V*sin(psi), gyro.psi_dot, 0;
        state = state + dt * u;
        state(2) = wrapAngle(state(2)); // Wrap the Heading Angle 

        MatrixXd F = Matrix4d::Zero();
        double dt_cos = dt*cos(psi);
        double dt_sin = dt*sin(psi);
        F << 1,0,-dt*V*dt_sin,dt_cos,
             0,1,dt*V*dt_cos,dt_sin,
             0,0,1,0,
             0,0,0,1;
