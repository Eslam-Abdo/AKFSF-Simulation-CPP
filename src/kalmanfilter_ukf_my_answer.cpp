...
std::vector<VectorXd> generateSigmaPoints(VectorXd state, MatrixXd cov)
{
    ...
    double lamda = 3.0 - numStates;
    MatrixXd sqrtCov = cov.llt().matrixL();
    sigmaPoints.push_back(state);
    for(int iState=0; iState < numStates; iState++)
    {
        sigmaPoints.push_back(state + sqrt(numStates + lamda) * sqrtCov.col(iState));
        sigmaPoints.push_back(state - sqrt(numStates + lamda) * sqrtCov.col(iState));
    }
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        std::vector<VectorXd> sigmaPointsPredict;
        for (const auto& sigmaPoint : sigmaPoints)
        {
            sigmaPointsPredict.push_back(vehicleProcessModel(sigmaPoint,gyro.psi_dot,dt));
        }

        // Calculate Mean
        state = VectorXd::Zero(n_x);
        for(unsigned int i = 0; i < sigmaPointsPredict.size(); ++i)
        {
            state += sigmaWeights[i] * sigmaPointsPredict[i];
        }
        ...
        for(unsigned int i = 0; i < sigmaPointsPredict.size(); ++i)
        {
            VectorXd diff = normaliseState(sigmaPointsPredict[i] - state);
